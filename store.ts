...
interface GameState {
  ...
  resetTaps: () => void;
  syncTrigger: number;
  triggerSync: (startBeatIndex?: number) => void;
  videoId: string;
  ...
  setCameraFilter: (
    filter: "none" | "noir" | "chrome" | "thermal" | "comic" | "pastel",
  ) => void;
...
export const useGameStore = create<GameState>((set) => ({
  ...
  syncTrigger: 0,
  triggerSync: (startBeatIndex?: number) =>
    set((state) => {
      console.log(
        `[Store] triggerSync called${startBeatIndex !== undefined ? ` with startBeatIndex: ${startBeatIndex}` : ""}`,
      );
      return {
        syncTrigger: state.syncTrigger + 1,
        ...(startBeatIndex !== undefined && { currentBeat: startBeatIndex }),
      };
    }),
  ...
  breakBrick: (index: number) =>
    set((state) => ({
      brokenBricks: { ...state.brokenBricks, [index]: true },
    })),
  resetBricks: () => {
    set({ brokenBricks: {} });
  ...
  loadNewRhymes: (rowIndex?: number) => {
    ...
    const getNewGroup = () => {
      const useTarget =
        state.targetFamilyIds.length > 0 &&
        Math.random() * 100 < state.masteryPercentage;
      let family;
      if (useTarget) {
        const randomTargetId =
          state.targetFamilyIds[
            Math.floor(Math.random() * state.targetFamilyIds.length)
          ];
    ...
    if (typeof rowIndex === "number") {
      ...
      set((prev) => {
        ...
        if (newRows[rowIndex])
          newRows[rowIndex] = { ...newRows[rowIndex], word };
  ...
  shiftBoard: () => {
    set((state) => {
      ...
      if (state.rhymeScheme === "AABB") {
        ...
        state.rhymeScheme === "ABAB" ||
        state.rhymeScheme === "AXBX" ||
        state.rhymeScheme === "XAXB"
      ) {
